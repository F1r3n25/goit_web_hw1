...
def info_command():
    print("Available commands: 'record', 'upload', 'add number', 'add email', 'upcoming', 'find', 'remove', 'edit', 'save', 'open'")
...
class SavedAddressbook(AddressBook):
    def upload(self):
        ...
        with open("Addressbook.txt","a",encoding="utf-8") as f:
            ...
            for key, el in self.data.items():
                f.writelines("{:<18} {:<10}\n".format("Name:", el["Name"]))
                f.writelines("{:<18} {:<10}\n".format("Address: ", el["Address"]))
                f.writelines("{:<18} {:<10}\n".format("Phone number(s):", ", ".join(el["Number"])))
                f.writelines("{:<18} {:<10}\n".format("Email address(es):", ", ".join(el["Email"])))
                f.writelines("{:<18} {}\n".format("Birthdate:", el["Birthdate"]))
                if len(self.data) > 1 and key != last_key:
                    f.writelines("************************************\n")
            f.writelines("------------------------------------\n")
        print("Data was successfully saved")
...
ab = PrintedAddressbook() # Екземпляр адруссбука, який виводить картку на екран
ab = SavedAddressbook() # Екземпляр адруссбука, який зберігає картку в файл
ab = SpokedAddressbook() # Екземпляр адруссбука, який озвучую картку контакту
...
commands = {
    ...
    ab.upload: "upload",  # Перегляд одразу всього адрессбуку
